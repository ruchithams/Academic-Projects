...
public class OthelloSolver {
	...
	static float minimax_value(int board[][], boolean whiteTurn, int searchDepth, float alpha, float beta) {
		...
		if( whiteTurn)
		{
			...
			{
				float res = minimax_value(play(board, move, whiteTurn), false,searchDepth-1, alpha, beta);
		...
		else 
		{
			...
			for(Move move: generateLegalMoves(board, whiteTurn))
			{
				float res = minimax_value(play(board, move, whiteTurn), true,searchDepth-1, alpha, beta);
				minVal = Math.max(res, minVal);

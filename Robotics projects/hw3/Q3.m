...
function [Abar,Bbar,QTbar,Qbar,x0bar] = getSyntheticDynamics(A,B,QT,Q,R,x0)
    % System parameters for optimized value
    Abar = [A B; zeros(2,4) eye(2)];
    Bbar = [B; eye(2)];
    QTbar =  [QT zeros(4,2); zeros(2,4) zeros(2,2)];
    Qbar = [Q zeros(4,2); zeros(2,4) R];
    % calculating Pseq for 100 timesteps
    Pseq = FH_DT_Riccati(Abar,Bbar,QTbar,Qbar,R,100);
    X = [x0;0;0;];
    % calculating new control value
    u0 = -(inv(R +(Bbar'* Pseq(:,:,1) * Bbar)) * (Bbar'* Pseq(:,:,1) *Abar * X));
    % computing new initial state
    x0bar = (Abar * X) + (Bbar * u0);
end
...
function Pseq = FH_DT_Riccati(A,B,Qf,Q,R,T)
   Pseq(:,:,T) = Qf;
   % inserting p matrices to cell array
   for i= T-1:-1:1
       Pnext = Pseq(:,:,i+1);
